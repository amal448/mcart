...
module.exports = {
    ...
    addProductOffer: (data) => {
        return new Promise(async (resolve, reject) => {
            ...
            let exist = await db.get().collection(collection.PRODUCTOFFER_COLLECTION).findOne({ product: data.product });
            console.log(exist,"111")
            if (exist) {
                reject();
                console.log("222")
            }
                ...
                let exist = await db.get().collection(collection.PRODUCT_COLLECTION).findOne({ Name: data.Name, offer: { $exists: true } });

                if (exist) {
                    console.log("333",exist)
                    reject();
                } else {
                    db.get().collection(collection.PRODUCTOFFER_COLLECTION).insertOne(data).then(() => {
                        resolve();
                    })
                }
            }
    ...
    editProdOffer: (proOfferId, data) => {
        return new Promise((resolve, reject) => {
            console.log(proOfferId)
            db.get().collection(collection.PRODUCTOFFER_COLLECTION).updateOne(
                ...
                let products = await db.get().collection(collection.PRODUCT_COLLECTION).find({ product: data.Name, offer: { $exists: true } }).toArray();
    ...
    deleteProdOffer: (proOfferId) => {
        return new Promise(async (resolve, reject) => {
            let productOfferPercentage = await db.get().collection(collection.PRODUCTOFFER_COLLECTION).findOne({ _id: objectId(proOfferId) })
            console.log('prodoff = ',productOfferPercentage);
            var pname = null
            if(productOfferPercentage){
                 pname = productOfferPercentage.product;
            }
